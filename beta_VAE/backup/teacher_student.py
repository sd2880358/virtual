...
def start_train(epochs, student_model, teacher_model, full_range_set, partial_range_set, date, filePath):
    ...
    ckpt = tf.train.Checkpoint(model=student_model,
                               optimizer=optimizer)
    ...
    generate_and_save_images(student_model, 0, test_sample, file_path)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            for i in range(10, 360, 10):
                ...
                ori_loss = compute_loss(student_model, test_sample)
                rota_loss = reconstruction_loss(student_model, test_sample)
                ori_cross_l = ori_cross_loss(student_model, test_sample, d, r_x)
                rota_cross_l = rota_cross_loss(student_model, test_sample, d, r_x)
                total_loss = ori_loss + rota_loss + ori_cross_l + rota_cross_l
                loss(total_loss)

            elbo = -loss.result()
            generate_and_save_images(student_model, epoch, test_sample, file_path)
